...
CUHF::CUHF(Process::Environment& process_environment_in, Options& options, boost::shared_ptr<PSIO> psio, boost::shared_ptr<Chkpt> chkpt) : HF(process_environment_in, options, psio, chkpt)
{
    common_init();
}

CUHF::CUHF(Process::Environment& process_environment_in, Options& options, boost::shared_ptr<PSIO> psio) : HF(process_environment_in, options, psio)
{
    common_init();
}
    ...
    if (save_diis){
        if (initialized_diis_manager_ == false) {
            diis_manager_ = boost::shared_ptr<DIISManager>(new DIISManager(
                                                               process_environment_, max_diis_vectors_, "HF DIIS vector", psio_, DIISManager::LargestError,
                                                               DIISManager::OnDisk));
