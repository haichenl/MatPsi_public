...
void HF::common_init()
{
    ...
    EnvMat_.reset(factory_->create_matrix("One-electron environmental Hamiltonion"));
}
...
void HF::set_EnvMat(SharedMatrix EnvMat_in) {
    ...
    if(EnvMat_in->nirrep()!= example->nirrep() || EnvMat_in->colspi()!= example->colspi() || EnvMat_in->rowspi()!=example->rowspi())
        throw PSIEXCEPTION("HF::set_EnvMat(EnvMat_in): Input environmental Hamiltonian dimensions do not agree.");
    // If dimensions agree then enable EnvMat_ 
    EnvMat_enabled_ = true;
    EnvMat_ = EnvMat_in;
}
