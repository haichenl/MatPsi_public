...
double HF::compute_energy(SharedMatrix EnvMat)
{
    // Check the dimensions of input environment potential energy matrix 
    if(EnvMat->nirrep() != 1 || EnvMat->nrow() != basisset_->nbf() || EnvMat->ncol() != basisset_->nbf())
        throw PSIEXCEPTION("HF::compute_energy(EnvMat): Environment potential energy matrix dimensions do not agree.");
    ...
    if(attempt_number_ == 1){
        ...
        form_H(); //Core Hamiltonian
        H_->add(EnvMat); // Add environment in 
